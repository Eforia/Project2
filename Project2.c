...
void init_catalog(){ 
    product catalog[TABLE_SIZE] =   {
        ...
        {"Rice", 2, 3.99, 0, 0},
        {"Pasta", 2, 1.79, 0, 0},
        {"Tomato Sauce", 2, 2.49, 0, 0},
        {"Cereal", 2, 3.99, 0, 0},
        {"Orange Juice", 2, 4.29, 0, 0},
        {"Coffee", 2, 8.99, 0, 0},
        {"Tea", 2, 3.50, 0, 0},
        {"Potatoes", 2, 2.99, 0, 0},
        {"Onions", 2, 1.89, 0, 0},
...
void order(int product_id,int write_fd)
{
    ...
    //using snprintf we can store a message in the answer variable and write that the pipe
...
int main (int argc,char **argv)
{
    ...
    {   
        ...
        if (ppid[i] == 0)
        {
            ...
            {
                int product_id = rand() % TABLE_SIZE; //picks random number based on table size
                write(fd[i][1], &product_id,sizeof(product_id)); //writes product id to pipe
                sleep(1);
            }
    ...
    for(int i = 0; i < TABLE_SIZE; i++)
    {
        printf("Product: %s , was requested a total of: %d times with %d units sold and %d failed purchases due to insufficient stock\n", catalog[i].description, catalog[i].req, 2 - catalog[i].item_count, catalog[i].freq);
        final.total_sales += catalog[i].req;
        final.failed_s += catalog[i].freq;
    }

    printf("The total number of attempted sales was: %d, of those %d were completed while %d failed totaling %f euros \n", final.total_sales, final.total_sales - final.failed_s, final.failed_s, final.total);
